...
export function RadarPerfChart({ data }: RadarPerfChartProps) {
  ...
  return (
    <ChartPlot style={{ height: CARD_CHART_HEIGHT_RADAR, minHeight: CARD_CHART_HEIGHT_RADAR }} className="w-full">
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          <PolarAngleAxis
            dataKey="metric"
            tick={{ ...axisTick(theme), fontSize: 11 }}
            tickLine={false}
          />
          <PolarRadiusAxis
            domain={domain}
            tickCount={4}
            tickFormatter={fmt3}
            tick={{ ...axisTick(theme), fontSize: 9 }}
